...
def test_order_mark_and_update():
    order = Order(local_order_id=1, side=Order.SIDE_BUY, type=Order.TYPE_LIMIT,
                  amount=10.0, price=100.0, symbol="BTC/USDT")
    order.mark_as_placed_on_exchange("ex1", 1234567890)
    ...
    order.update_from_ccxt_response(data)
    assert order.filled == 5
    assert order.remaining == 5
    assert order.average == 101.0
    ...
    order.update_from_ccxt_response(data_filled)
    assert order.is_filled()
    assert order.get_fill_percentage() == 1.0


def test_order_dict_roundtrip():
    order = Order(local_order_id=2, side=Order.SIDE_SELL, type=Order.TYPE_MARKET,
                  amount=3.0, price=0.0, symbol="ETH/USDT")
    order.mark_as_placed_on_exchange("ex2")
