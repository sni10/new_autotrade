...
class OrderFactory:
    ...
    def __init__(self):
        pass  # здесь можно прокидывать какие-то настройки по умолчанию

    def create_buy_order(self, price: float, amount: float) -> Order:
        order_id = int(time.time() * 1000)  # или любой уникальный ID
        return Order(
            order_id=order_id,
            side=Order.SIDE_BUY,
            order_type=Order.TYPE_LIMIT,  # пока жёстко LIMIT для примера
            price=price,
            amount=amount,
